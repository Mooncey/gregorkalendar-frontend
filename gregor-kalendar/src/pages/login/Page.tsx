...
export default function LoginPage() {
  ...
  const [fetchParams, setFetchParams] = useState<{ userEmail: string }>({ userEmail:"" });
  const [triggerFetch, setTriggerFetch] = useState(false);

  const { data, isLoading, error } = useFetch(getUser, fetchParams, triggerFetch);


  const HandleSignIn = () => {

    setFetchParams({ userEmail: email });
